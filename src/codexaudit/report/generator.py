...
from codexaudit.models import ReportData
...
def generate_markdown_report(report: ReportData) -> str:
    ...
    lines.append("# Codexaudit Report")
    lines.append("")
    lines.append(f"**Project**: `{report.project_path}`")
    lines.append(f"**Generated**: {report.generated_at.strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("**codexaudit v0.1.0**")
    lines.append("")
    ...
    # Footer
    lines.append("---")
    lines.append("*Generated by codexaudit v0.1.0*")
...
def write_reports(report: ReportData, output_dir: Path) -> tuple[Path, Path]:
    ...
    md_path = output_dir / "codexaudit-report.md"
    json_path = output_dir / "codexaudit-report.json"
